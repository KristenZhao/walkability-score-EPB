...
CREATE TABLE jz_handover.hkmaw_32650_poi_resultspgr AS
WITH all_nodes as(
SELECT * FROM pgr_withPointsDD(
'SELECT id AS id, source, target, cost
 FROM jz_handover.hkmaw_32650_edges_1_noded ',
'SELECT id AS pid, edge_id, fraction
 FROM jz_handover.hkmaw_32650_poi_1_noded WHERE edge_id IS NOT NULL',
/*the following line selects the first 10,000 junctions as starting points.*/
 (SELECT ARRAY (SELECT id FROM jz_handover.hkmaw_32650_edges_1_noded_vertices_pgr ORDER BY id)),400,
 directed := false,
 details := true)
)
...
FROM all_nodes AS n
INNER JOIN jz_handover.hkmaw_32650_poi_1_noded as p
on n.node = p.pid;
/* this takes  1 secs 225 msec */

-- FINDING THE NODES WITH NO POIS ASSOCIATED WITH
CREATE TABLE jz_handover.hkmaw_32650_edges_1_noded_vertices_no_pois AS
SELECT id, the_geom FROM jz_handover.hkmaw_32650_edges_1_noded_vertices_pgr AS t1
	WHERE NOT EXISTS(SELECT id FROM jz_handover.hkmaw_32650_poi_resultspgr AS t2 WHERE t1.id = t2.id);
...
ALTER TABLE jz_handover.hkmaw_32650_poi_resultspgr ADD COLUMN cates VARCHAR(20);
UPDATE jz_handover.hkmaw_32650_poi_resultspgr 
	SET cates = 
