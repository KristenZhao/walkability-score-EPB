--create a new table with only 32650 projection edges in hki TO be used for creating topology
CREATE TABLE jz_handover.hkhki_32650_edges_1 AS
SELECT id, id_cont, type, levels, ROUND(shape_leng,2) AS cost, source, target, null_geom, geom_32650 AS geom, area
	FROM jz_handover.hk_102140_edges WHERE null_geom = 0 AND area = 'hki';

--create topology
SELECT pgr_createTopology('jz_handover.hkhki_32650_edges_1', 1, 'geom');

-- change cost type to double precision
ALTER TABLE jz_handover.hkhki_32650_edges_1 ALTER COLUMN cost TYPE double precision;

--create a new poi table with only poi in hki
CREATE TABLE jz_handover.hkhki_32650_poi_1 AS
...
SELECT * FROM jz_handover.hkhki_32650_poi_1;
...
WITH pe AS ( SELECT DISTINCT ON(p.id) p.id,
   ...
   FROM jz_handover.hkhki_32650_poi_1 As p INNER JOIN jz_handover.hkhki_32650_edges_1 As e
        ON ST_DWithin(e.geom, p.geom, 500)
     ORDER BY p.id, ST_Distance(e.geom, p.geom) )
UPDATE jz_handover.hkhki_32650_poi_1 AS p
